...
export async function POST(request: NextRequest) {
  try {
    ...
    const { nik, nama, jabatan, telepon, alamat } = body

    // Validation
    if (!nik || !nama || !jabatan) {
      return NextResponse.json(
        { success: false, error: 'NIK, nama, dan jabatan wajib diisi' },
        { status: 400 }
      )
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (isInUse) {
      return NextResponse.json(
        { success: false, error: 'Karyawan tidak dapat dihapus karena masih memiliki transaksi aktif' },
        { status: 400 }
      )
    }
